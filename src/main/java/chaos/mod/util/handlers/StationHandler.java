...
public class StationHandler {
	...
	private void register(File file, int index, RegisterType type) {
		...
		try (FileReader reader = new FileReader(file)) {
			...
			case STA:
				...
				if (!json.get("lines").isJsonNull()) {
					JsonArray arr = json.get("lines").getAsJsonArray();
					for (int i = 0; i < arr.size(); i++) {
						sta.addLine(new Line(arr.get(i).getAsString()));
					}
				}
	...
	private void save(Station sta) throws IOException {
		...
		if (!sta.getLines().isEmpty())
			for (Line line : sta.getLines()) {
				arr.add(line.getName());
			}
