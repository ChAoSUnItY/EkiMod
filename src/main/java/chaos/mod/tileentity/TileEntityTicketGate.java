...
public class TileEntityTicketGate extends TileEntity {
	...
	public int[] getAnchorPos() {
		return hasAnchorPos ? new int[] { this.PosX, this.PosZ, this.PosY }
				: new int[] { pos.getX(), pos.getZ(), pos.getY() };
	}
	...
	public boolean isAnchorExists() {
		if (!hasAnchorPos)
			return false;
		return worldObj.getTileEntity(new BlockPos(this.PosX, this.PosY, this.PosZ)) instanceof TileEntityAnchor;
	}
