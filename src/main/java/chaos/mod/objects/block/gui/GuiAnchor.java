...
public class GuiAnchor extends GuiScreen {
	...
	public void initGui() {
		...
		x = width / 2 - 90;
		y = height / 2 - 100;
		nameText = new GuiTextField(0, fontRendererObj, x + 70, y + 50, 100, 15);
		OPText = new GuiTextField(1, fontRendererObj, x + 177, y, 100, 15);
		...
		playerList = new GuiSimpleListBox(x + 177, y + 41, 100, 125,
				(List<? extends DataForm>) (mc.isSingleplayer() ? Lists.newArrayList() : UtilDataForm.toDataForm(UtilDataForm.getPlayersName(mc.getConnection().getPlayerInfoMap()))));
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		OPText.drawTextBox();
		// draw string
		fontRendererObj.drawString(BlockInit.ANCHOR.getLocalizedName(), (x + 176 / 2) - (fontRendererObj.getStringWidth(BlockInit.ANCHOR.getLocalizedName()) / 2), y + 8, 4210752);
		fontRendererObj.drawSplitString(new UtilTranslatable(TranslateType.CONTAINER, "anchor.station.name").getFormattedText() + (te.isValidStation() ? te.getStation().getName() : ""), x + 70, y
				+ 20, 106, 4210752);
		fontRendererObj.drawSplitString(new UtilTranslatable(TranslateType.CONTAINER, "anchor.station.class").getFormattedText()
				+ (te.isValidStation() ? StationHandler.INSTANCE.getStation(te.getPos()).getLvl().getFormattedText() : ""), x + 5, y + 55, 66, 4210752);
		fontRendererObj.drawSplitString(new UtilTranslatable(TranslateType.CONTAINER, "anchor.desc").getFormattedText(), x + 5, y + 93, 166, 4210752);
		// draw list
		playerList.draw(mouseX, mouseY, fontRendererObj);
		staLvlList.draw(mouseX, mouseY, fontRendererObj);
	}
