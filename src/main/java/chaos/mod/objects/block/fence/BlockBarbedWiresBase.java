...
public class BlockBarbedWiresBase extends BlockHorizontalBase {
	...
	public BlockBarbedWiresBase(String name, BarbedWireType type) {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (stack.getItem() instanceof ItemBlock) {
			...
			if (itemblk.getBlock() instanceof BlockBarbedWiresBase && getType() == BarbedWireType.BASE) {
				for (;;) {
					IBlockState s = worldIn.getBlockState(pos);
					if (s.getBlock() instanceof BlockBarbedWiresBase) {
						BlockBarbedWiresBase blk = (BlockBarbedWiresBase) s.getBlock();
