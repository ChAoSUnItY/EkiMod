...
public class ItemBarbedWireSet extends ItemBase {
	...
	public ActionResult<ItemStack> onItemRightClick(ItemStack itemStackIn, World worldIn, EntityPlayer playerIn, EnumHand hand) {
		if (worldIn.isRemote)
			Eki.proxy.displayBarbedWireSetConfig(playerIn.getHeldItem(hand), playerIn, hand);
	...
	public EnumActionResult onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (itemstack.stackSize != 0 && playerIn.canPlayerEdit(pos, facing, itemstack) && worldIn.canBlockBePlaced(BlockInit.BARBED_WIRES_BASE, pos, false, facing, playerIn, itemstack)) {
			IBlockState state = BlockInit.BARBED_WIRES_BASE.getDefaultState();

			if (placeBlockAt(itemstack, playerIn, worldIn, pos, facing, hitX, hitY, hitZ, state)) {
				state = worldIn.getBlockState(pos);
				SoundType soundtype = state.getBlock().getSoundType(state, worldIn, pos, playerIn);
