...
public class ItemRangeFinder extends ItemBase {
	...
	public boolean onItemUseFirst(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
		if (!world.isRemote) {
			...
			if (stack.hasTagCompound() && stack.getTagCompound().hasKey("startPos")) {
				...
				text = new ChatComponentTranslation("chat.type.text.rangefinder.result", ThreeDimLength, TwoDimLength);
				...
				stack.setTagCompound(nbt);
				text = new ChatComponentTranslation("chat.type.text.rangefinder.getpos", x, y, z);
	...
	public void addInformation(ItemStack stack, EntityPlayer playerIn, List tooltip, boolean advanced) {
		...
		tooltip.add(EnumChatFormatting.GRAY + I18n.format(getUnlocalizedName() + ".tooltip", hasTag ? stack.getTagCompound().getIntArray("startPos")[0]
				: null, hasTag ? stack.getTagCompound().getIntArray("startPos")[1] : null, hasTag ? stack.getTagCompound().getIntArray("startPos")[2] : null));
	}
}
